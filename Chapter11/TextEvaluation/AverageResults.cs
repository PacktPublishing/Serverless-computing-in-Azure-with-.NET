...
namespace TextEvaluation
{
	public static class AverageResults
	{
		...
		public static async void Run([TimerTrigger("0 * * * * *")]TimerInfo myTimer,
			...
			TraceWriter log)
		{
			...
			var today = XmlConvert.ToString(DateTime.Now, 
        XmlDateTimeSerializationMode.Utc);
			...
			if (entities.Items.Count() == 1)
			{
				...
				log.Info($"Updating average sentiment score " +
          $"for today to average={average}");
			...
			{
				// Create new entry
				log.Info($"Creating new average sentiment score " +
          $"for today average={average}");
